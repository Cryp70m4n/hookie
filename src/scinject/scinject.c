...
void convertShellcode(char *shellcodeString, unsigned char *shellcode) {
    int length = strlen(shellcodeString);
		int i = 2;
		int k = 0;	
		// we can define it as 3 because it always contains 2 digits + null terminator (Added null terminator just in case)
...
void printUsage(void) {
		printf("\033[0;34m[?] => USAGE: scinject --pid PID --shellcode \"\\x42\\x42\\x42\"\033[0m\n");
}
