...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    printf(
        "\033[0;31m[X] >>> Insufficient number of "
  ...
  printf("(\033[0;34mPID\033[0m) -> [\033[0;33m%d\033[0m] <<<< | >>>> "
         "[\033[0;33m%s\033[0m] <- (\033[0;34mNAME\033[0m)\n",
         pData->pid, pData->name);

  int k = 0;
  while (loadedSharedObjects[k] != NULL) {
    printf("(\033[0;32mSHARED OBJECT\033[0m) -> "
           "[\033[0;31m%s\033[0m]\n\t(\033[0;35mVERSION\033[0m) -> "
           "[\033[0;36m%s\033[0m]\n\t\t(\033[0;34mBASE ADDRESS\033[0m) -> "
           "[\033[0;33m0x%lx\033[0m]\n",
           loadedSharedObjects[k]->name, loadedSharedObjects[k]->version,
           loadedSharedObjects[k]->baseAddress);
    k++;
  }
