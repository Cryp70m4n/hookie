...
SharedObject **parseLoadedSharedObjects(int pid) {
  ...
  SharedObject **sharedObjects =
      malloc(MAX_SHARED_OBJECTS * sizeof(SharedObject *));
  ...
  while (fgets(line, MAX_LINE_LENGTH, mapsFile) != NULL) {
    SharedObject *sharedObject =
        malloc(MAX_SHARED_OBJECT_LENGTH * sizeof(SharedObject));
    ...
    while (line[i] != '/' && line[i] != '\t') {
      // we can do if line[i-1] without checking if i > 0 because in maps / or
      // \t will always appear before index 0
    ...
    char *versionString =
        malloc(MAX_SHARED_OBJECT_VERSION_LENGTH * sizeof(char));
    ...
    while (i < strlen(line) - 1) {
      if (k > 4) {
        if ((tmpString[k - 3] == '.' && tmpString[k - 2] == 's' &&
             tmpString[k - 1] == 'o') &&
    ...
    while (sharedObjects[tmp] != NULL) {
      if (!strcmp(sharedObjects[tmp]->name, sharedObject->name) &&
          !strcmp(sharedObjects[tmp]->version, sharedObject->version)) {
